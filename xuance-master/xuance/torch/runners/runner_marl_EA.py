...
from xuance.ea import mod_neuro_evo as utils_ne


class Runner_MARL_EA(Runner_Base):
    def __init__(self, args):
        super(Runner_MARL_EA, self).__init__(args)
        self.args = args
    ...
    def store_data(self, obs_n, next_obs_n, actions_dict, state, next_state, agent_mask, rew_n, done_n):
        data_step = {'obs': obs_n, 'obs_next': next_obs_n, 'actions': actions_dict['actions_n'], 'state': state, 'state_next': next_state, 'rewards': rew_n,
    ...
    def train_steps(self, n_steps):
        ...
        state = self.envs.global_state()
        agent_mask = self.envs.agent_mask()
        for _ in tqdm(range(n_steps)):
            step_info = {}
            actions_dict = self.get_actions(obs_n, False, act_mean_last, agent_mask, state)  # 动作
            ...
            for i_env in range(self.n_envs):
                if terminated_n.all(axis=-1)[i_env] or truncated_n.all(axis=-1)[i_env]:
                    ...
                    if self.use_wandb:
                        ...
                        # 自定义
                        step_info["jain_f_m/env-%d" % i_env] = np.mean(infos[i_env]["jain_f_m"])
                    else:
                        step_info["Episode-Steps"] = {"env-%d" % i_env: infos[i_env]["episode_step"]}
                        step_info["Train-Episode-Rewards"] = {"env-%d" % i_env: np.mean(infos[i_env]["episode_score"])}
                        # 自定义
