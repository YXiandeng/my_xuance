    ...
    def test_episodes(self, env_fn, test_episodes):
        ...
        while current_episode < test_episodes:
            ...
            for i_env in range(num_envs):
                # 自定义获取环境中无人机位置坐标
                for uav_i in range(obs_n.shape[1]):
                    step_info["uav-station-%d-x" % uav_i] = infos[i_env]["uav-station-%d-x" % uav_i]
                    step_info["uav-station-%d-y" % uav_i] = infos[i_env]["uav-station-%d-y" % uav_i]
                # self.log_infos(step_info, self.current_step)  # x轴，y轴
                # self.current_step +=1
                if terminated_n.all(axis=-1)[i_env] or truncated_n.all(axis=-1)[i_env]:
                    obs_n[i_env] = infos[i_env]["reset_obs"]
                    agent_mask[i_env] = infos[i_env]["reset_agent_mask"]
